 ...
 */
public class FunctionLab2Q1B extends Function
{
    ...
    public float getY(float x, float[] ys, int func, boolean actual)
    {
        switch(func)
        {
            case 0: return actual ? (float)(2*Math.exp(-1*x) - 2*Math.exp(-2*x) + Math.sin(x)) // actual
                                  : (float)(-4*ys[0] - 2*ys[1] + Math.cos(x) + 4*Math.sin(x)); // guess
            case 1: return actual ? (float)(-3*Math.exp(-x) + 2*Math.exp(-2*x)) // actual
                                  : (float)(3*ys[0] + ys[1] - 3*Math.sin(x)); // guess
            default: return 0;
        }
    }
}
