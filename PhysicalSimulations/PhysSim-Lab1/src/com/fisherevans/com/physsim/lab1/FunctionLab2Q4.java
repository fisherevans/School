 ...
 */
public class FunctionLab2Q4 extends Function
{
    ...
    public XYDataset getDataset()
    {
        ...
        for(double x = startTime;x <= startTime+duration && Math.abs(ys[0]) < 39000;x += h)
        {
            for(int func = 0;func < n;func++)
            {
                if(func == 1)
    ...
    private double M = 70;
    private double A = 0.012;
    private double PO = 1.225;
    private double H = 7*1000;

    public double getY(double x, double[] ys, int func, boolean actual)
    {
        switch(func)
        {
            case 0: return actual ? 0 // actual
                    : ys[1]; // guess
            case 1:
                    return actual ? -G*x // actual
                    : -G + PO*Math.exp(-(39000+ys[0])/H)*CD*ys[1]*ys[1]*A/2.0; // guess
            default: return 0;
        }
    }
}
