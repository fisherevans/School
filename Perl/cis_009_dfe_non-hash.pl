...
@charArray = ( # Define the character array, and init the count to 0 - Evens are chars / Odds are count
...
while (<INPUTFILE>) # loop through each line
{
	for($i = 0;$i < length($_);$i++) # each char in that line
	{
		...
		if($tempChar ne "\n") # if it's not a new line
		{
			...
			if ($t > $#charArray) # add new char to list and set initial cnt to 1 - RHoule
			{
				push (@charArray, $tempChar); push (@charArray , 1);
			}
		}
	}
}
