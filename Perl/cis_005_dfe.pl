...
$minPayment = $loanSize * $mpr; # Find minimum payment
...
{
	$totalOwed = $loanSize; #cost to pay over time
	for($loop = 0;$totalOwed > 0;$loop++) # loop through each month until they've paid it all off
	{
		$totalOwed = $totalOwed + ($totalOwed*$mpr) - $payment;
	}
	
	$lastPayment = $paid - $totalCost; # find out cost of last payment.
	$lastPaymenr -= $payment;
	
	printf "It will take %d months to pay off a loan started as \$%.2f with a %%%.2f APR.\n", $loop-1, $loanSize, $apr; # Print out info
